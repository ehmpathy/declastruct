...
- Proper DAO implementation with:
  ...
  - `set.finsert()` - creates new resource on disk
...
const changes = await Promise.all(
  input.resources.map((resource) =>
    bottleneck.schedule(async () => {
      ...
      const symbol = change.action === DeclastructChangeAction.KEEP ? '↓' : '○';
      context.log.info(
...
for (const change of input.plan.changes) {
  if (change.action === DeclastructChangeAction.KEEP) {
    // log KEEP actions
    context.log.info(
      `↓ [KEEP] ${change.forResource.class}:${change.forResource.slug}`,
    );
    continue;
  }
  ...
  context.log.info(
    `✔ [${applied.action}] ${applied.forResource.class}:${applied.forResource.slug}`,
  );
...
log.info('');
log.info(`🌊 applied ${appliedChanges.length} changes`);
```
...
- `src/contract/cli/__test_assets__/wish.fixture.ts` → `.test/assets/wish.fixture.ts`
...
### Unit Tests
```
✓ getDaoByResource (3 tests)
✓ computeChange (5 tests)
✓ applyChange (6 tests)
```

### Integration Tests
```
✓ planChanges.integration (4 tests)
...
✓ applyChanges.integration (5 tests)
...
- ✅ Immutable variables (const only)
- ✅ Fail-fast error handling with HelpfulErrors
- ✅ Comment discipline (.what, .why, .note)
- ✅ Code paragraphs with explanatory comments
- ✅ No mocking in integration tests
- ✅ Real on-disk persistence for demo provider
- ✅ Domain-driven design with DomainEntity usage
- ✅ Proper bounded contexts and directional dependencies
