...
│   ├── DeclastructProvider.ts
│   ├── DeclastructDao.ts
│   ├── DeclastructChange.ts
│   └── DeclastructPlan.ts
│
├── domain.operations/       # business logic and orchestration
│   ├── plan/
│   │   ├── changes.ts
│   │   └── validate.ts
│   └── apply/
...
- `contract/` → can depend on `domain.operations/`, `domain.objects/`, `infra/`
- `domain.operations/` → can depend on `domain.objects/`, `infra/`
- `domain.objects/` → standalone, no upward dependencies
- `infra/` → standalone, no dependencies (pure infrastructure adapters)
...
const applyChange = async ({
  ...
  switch (change.action) {
    ...
    case DeclastructChangeAction.CREATE:
      // create new resource
      await dao.set.finsert(change.state.desired);
      return change;
...
export const executePlanCommand = async (
  ...
  // log summary
  console.log()
  console.log(`🌊 planned for ${plan.changes.length} resources`);
  ...
  for (const change of plan.changes) {
    ...
    try {
      ...
      if (change.action !== DeclastructChangeAction.KEEP) {
        ...
        appliedChanges.push(applied);
        console.log(`✔ [${change.action}] ${resourceSlug}`);
      } else {
        console.log(`↓ [${change.action}] ${resourceSlug}`);
      }
    } catch (error) {
...
### Planning
- Multiple `plan` calls with same input → same output plan
...
### DAO Operations
- `finsert`: find or insert - returns existing resource if found by unique keys, otherwise creates (idempotent)
...
const applyChange = async (input, context) => {
  ...
  switch (input.change.action) {
    case DeclastructChangeAction.CREATE:
      // create new resource
      return await dao.set.finsert(input.change.toDesiredState, context);
...
### Integration Tests
- Full plan → apply cycle with in-memory provider
