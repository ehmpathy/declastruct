...
- [ ] Implement `set.finsert`, `set.upsert`, `set.delete` methods
...
expect(mockDao.set.finsert).toHaveBeenCalledWith(createChange.state.desired);
...
- Logs each change: `○ [ACTION] resource:Class:slug`
- KEEP actions show `↓` symbol
...
# Verify output
# Should show:
# ○ [CREATE] resource:TestResource:test-1
# ↓ [KEEP] resource:TestResource:test-2
#
# 🌊 planned for 2 resources
...
- [ ] Show `✔` for success, `↓` for KEEP, `✖` for errors
...
- Success: `✔ [CREATE] resource:Class:slug`
- Skip: `↓ [KEEP] resource:Class:slug`
- Error: `✖ [CREATE] resource:Class:slug` (then throws)
...
**Tasks:**
- [ ] Create `src/__test__/fixtures/InMemoryProvider.ts`
- [ ] Implement in-memory DAOs for test resources
- [ ] Support all DAO operations (get, finsert, upsert, delete)
...
// Create
const created = await dao.set.finsert(resource);
...
**Phase 1:** ☐ All domain objects implemented and tested
**Phase 2:** ☐ Reference type system complete
**Phase 3:** ☐ Context types defined
**Phase 4:** ☐ Supporting operations implemented
**Phase 5:** ☐ Core operations (plan/apply) working
**Phase 6:** ☐ CLI commands functional
**Phase 7:** ☐ SDK properly exported
...
2. ✅ Integration test demonstrates full plan → apply → re-plan → re-apply cycle
3. ✅ CLI commands work end-to-end
4. ✅ Example provider exists and is documented
5. ✅ No `any` types in public API
6. ✅ All code follows mechanic style briefs (immutability, fail-fast, arrow functions, comments)
7. ✅ TypeScript compiles with no errors
8. ✅ ESLint passes with project rules
